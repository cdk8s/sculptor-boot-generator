...
public class GeneratorForeignKeyOneToQueryParam implements GeneratorStrategy {

	@Override
	public void generatorFile(VelocityContext context, Configuration config) {
		String template = "templates/velocity/backend/param/ForeignKeyOneToQueryParam.java.vm";
		TableEntity tableEntity = (TableEntity) context.get("tableEntity");

		Object oneQueryParamColumns = context.get("foreignKeyColumns");
		if (null != oneQueryParamColumns) {
			List<ColumnEntity> columnEntityList = (List<ColumnEntity>) oneQueryParamColumns;
