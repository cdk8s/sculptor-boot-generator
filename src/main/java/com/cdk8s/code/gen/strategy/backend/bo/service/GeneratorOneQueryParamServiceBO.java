...
public class GeneratorOneQueryParamServiceBO implements GeneratorStrategy {

	@Override
	public void generatorFile(VelocityContext context, Configuration config) {
		String template = "templates/velocity/backend/bo/service/OneQueryParamServiceBO.java.vm";
		TableEntity tableEntity = (TableEntity) context.get("tableEntity");

		Object listQueryParamColumns = context.get("oneQueryParamColumns");
		if (null != listQueryParamColumns) {
			...
			for (ColumnEntity entity : columnEntityList) {
				...
				String fileName = getFileName(config, tableEntity.getUpperClassName(), entity.getUpperAttrName(), "ServiceBO");
